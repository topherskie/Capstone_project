<?php include 'inc/header_login_state.php'; ?>
<?php 
  
   $sesIdno . "<BR>";
  $sesUser . "<BR>";
   $sesUserType . "<BR>";
  $sesFname;
   $sesLname;

$row= '';
$img = '';
if(isset($_POST['btnCheckBio'])) {
  $txtRidno = $_POST['txtRidno'];

  //echo $txtRidno;

  $sqlCheckRecruit ="SELECT * FROM user WHERE user_idno='$txtRidno'";
 $result = $conn->query($sqlCheckRecruit);
$resultCheck = mysqli_fetch_array($result); 
$resultImg = $resultCheck['img_profile'];
$img  = "img_profiles/".$resultImg; 

}
?>



<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Recruiter Profile</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
  <style type="text/css">
    .card {
  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2);
  max-width: 300px;
  margin: auto;
  text-align: center;
}

.title {
  color: grey;
  font-size: 18px;
}

.btn-r {
  border: none;
  outline: 0;
  display: inline-block;
  padding: 8px;
  color: white;
  background-color: #000;
  text-align: center;
  cursor: pointer;
  width: 100%;
  font-size: 18px;
}

.pro-link {
  text-decoration: none;
  font-size: 22px;
  color: black;
}

.btn-r:hover, .pro-link:hover {
  opacity: 0.7;
}

.card-main-container {
  box-shadow: rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.23) 0px 6px 6px;
}

.sta {
  color:green;
}

  </style>

</head>
<body>

  <div class="card card-main-container">
  <img src="<?php echo $img; ?>" alt="<?php echo $resultCheck['user_fname']; ?>" style="width:100%">
   <h5 class="sta"><?php echo  $resultCheck['quality_status'] . ' ' .  $resultCheck['user_type_upgrade']; ?></h5>
  <h5><?php echo $resultCheck['user_fname']; ?></h5>
  <p class="title"><?php echo $resultCheck['user_email']; ?></p>
  
  <p><button class="btn-r">Player</button></p>
</div>


</body>
</html>