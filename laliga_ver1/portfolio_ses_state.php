...
$pUserIdno = '';
if(isset($_GET['pUserIdno'])) {
  $pUserIdno = $_GET['pUserIdno'];

  echo $pUserIdno;
} // end of the IF ELSE GET 


$sqlGetData = "SELECT * FROM player_stats WHERE user_idnoStats='$pUserIdno'";
...
// total points
$sqlTotalPoints = "SELECT SUM(stat_pts) AS total_points FROM player_stats WHERE user_idnoStats ='$pUserIdno'";
...
// TOTAL THE ROWS
$sqlToTalRow = "SELECT COUNT(user_idnoStats) AS total_rows FROM player_stats WHERE user_idnoStats='$pUserIdno'";
...
<body>
  ...
  <div class="main-score-container">
    <div><h3>Total pts made: <?php echo $resultTotalPoints ?></h3></div>;
    <div><h3>Total Pts Average: <?php echo round($sums, 2); ?></h3></div>
    <div>
    <a href="recruit_players_ses_state.php" class="btn btn-outline-primary">Players</a>
  </div>
  </div>
